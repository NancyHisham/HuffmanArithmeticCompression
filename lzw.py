...
class LZWCompressor:
    ...
    def lzw_compress(self, input_path, output_path):
        ...
        ratio = (comp_size / org_size) if org_size > 0 else 0 

        print(f"Compression complete!\nCompressed \033[35m{input_path}\033[0m")
        print(f"Original size: {org_size} bytes")
        print(f"Compressed size: {comp_size} bytes")
        print(f"Compression ratio: {ratio:.2f}")
        print(f"Compression time: {diff:.2f} seconds\n")
    ...
    def lzw_decompress(self, input_path, output_path):
        ...
        print(f"Decompression time: {diff:.2f} seconds\n")
...
if __name__ == "__main__":
    input_file = "HuffmanArithmeticCompression/data/powerpuff.png"
    compressed_file = "comp_powerpuff.lzw"
    decompressed_file = "decomp_powerpuff.png"
    
    lzw = LZWCompressor()

    lzw.lzw_compress(input_file, compressed_file)
    lzw.lzw_decompress(compressed_file, decompressed_file)
    lzw.verify(input_file, decompressed_file)
